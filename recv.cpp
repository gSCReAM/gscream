...
{
  GstPad *sinkpad;
  GstElement *videoconvertm = (GstElement *) data;

  /* We can now link this pad with the vorbis-decoder sink pad */
  g_print ("Dynamic pad created, linking decodebin and videoconvert\n");

  sinkpad = gst_element_get_static_pad (videoconvertm, "sink");
...
int main (int argc, char *argv[])
{
  ...
  GstElement *pipeline, *udpsrcm, *rtpdepaym, /**queuem,*/ *decodebinm, *videoconvertm, *autovideosinkm;
  GstBus *bus;
  guint bus_watch_id;
  GstCaps *udpcaps;
  ...
  udpsrcm = gst_element_factory_make("udpsrc","udpsrc");
  rtpdepaym = gst_element_factory_make("rtph264depay", "rtph264depay0");
  ...
  udpcaps = gst_caps_new_simple ("application/x-rtp",
    ...
    NULL);
  g_object_set(G_OBJECT(udpsrcm), "port", std::atoi(argv[1]),"caps", udpcaps, NULL);
  ...
  if(!gst_element_link_many (videoconvertm, autovideosinkm, NULL)){
    g_error("Could not link elements: videoconvert, autovideosink");
    return -1;
  }

  if(!g_signal_connect (decodebinm, "pad-added", G_CALLBACK (on_pad_added), videoconvertm)){
